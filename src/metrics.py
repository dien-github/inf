...
def get_target_from_data(data_path, dataset_name, size):
    ...
    if dataset_name == "rsud20k" or dataset_name == "grazpedwri-dx":  # YOLO format
        ...
        paths = [
            data_path + f"{os.sep}" + line.rstrip().replace("\\", f"{os.sep}")
            for line in open(data_path + f"{os.sep}test.txt")
        ]
        lb_paths = img2label_paths(paths)
        lb_dict = dict()
        for lb_file in lb_paths:
        ...
        paths = []
        class_lst = [
            line.rstrip() for line in open(data_path + f"{os.sep}class_names.txt")
        ]
        class_dict = dict()
        lb_dict = dict()
        ...
        for lb_file in lb_paths:
            lb_dict[lb_file.rsplit(f"{os.sep}")[-1].rsplit(".", 1)[0]] = get_label_voc(
                lb_file, class_dict
            )
        ...
        lb_paths = img2label_paths(
            paths, img_dir="val", lb_dir="annotations", lb_ext=".json"
        )

        lb_dict = dict()

        for lb_file in lb_paths:
            lb_dict[lb_file.rsplit(f"{os.sep}")[-1].rsplit(".", 1)[0]] = get_label_json(
                lb_file, class_dict
            )
        ...
        lb_dict = dict()
        for lb_file in lb_paths:
            lb_dict[lb_file.rsplit(f"{os.sep}")[-1].rsplit(".", 1)[0]] = get_label(
                lb_file
            )
        ...
        for img_id, anns in imgToAnns.items():
            ...
            for ann in anns:
                ...
                box = np.concatenate(
                    (np.array([cls], dtype=np.float32), segments2boxes(s)[0])
                )
                # s = [cls] + s
                # if box not in bboxes:
                bboxes.append(box)
        ...
        lb_paths = img2label_paths(
            paths, img_dir="images", lb_dir="semantic_masks", lb_ext=".png"
        )
        ...
        for lb_file in lb_paths:
            ...
            mask = np.stack([mask == 0, mask == 1, mask == 2], axis=-1).astype(
                np.float32
            )
        ...
        images_path = (
            data_path + f"{os.sep}segmentation-validationset{os.sep}val-org-img{os.sep}"
        )
        paths = []
        ...
        lb_paths = img2label_paths(
            paths, img_dir="val-org-img", lb_dir="val-label-img", lb_ext=".png"
        )
        ...
        lb_paths = img2label_paths(
            paths, img_dir="images_png", lb_dir="masks_png", lb_ext=".png"
        )
        ...
        lb_paths = img2label_paths(
            paths, img_dir="val", lb_dir="val_labels", lb_ext=".png"
        )

        class_lst = [
            line.rstrip() for line in open(data_path + f"{os.sep}class_dict.csv")
        ]
        ...
        for line in class_lst[1:]:
            parts = line.split(",")
            color_map.append(
                [int(parts[1].strip()), int(parts[2].strip()), int(parts[3].strip())]
            )
...
def metrics_np(
    ...
    intersection = np.sum(
        np.abs(y_pred * y_true), axis=axes
...
def box_iou(box1, box2, eps=1e-7):
    ...
    (a1, a2), (b1, b2) = (
        np.split(np.expand_dims(box1, 1), 2, 2),
        np.split(np.expand_dims(box2, 0), 2, 2),
    )
...
def eval_detection_results(results, nc, input_size):
    ...
    if len(stats) and stats[0].any():
        tp, fp, p, r, f1, ap, ap_class = ap_per_class(
            *stats, plot=False, save_dir=".", names=()
        )
...
def img2label_paths(img_paths, img_dir="images", lb_dir="labels", lb_ext=".txt"):
    ...
    """
    sa, sb = (
        f"{os.sep}{img_dir}{os.sep}",
        f"{os.sep}{lb_dir}{os.sep}",
    ...
    for shape in shapes:
        if shape["shape_type"] == "polygon":
            ...
            label = np.concatenate(
                (np.array([cls], dtype=np.float32), segments2boxes(segment)[0])
            )
            lb.append(label)
